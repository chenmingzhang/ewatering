...
#np.savetxt('layer_1_top_m_mtx.csv',layer_1_top_m_mtx,delimiter=',')

layer_1_top_m_mtx = np.loadtxt('layer_1_top_m_mtx.csv', delimiter=',')
...
#np.savetxt('layer_2_top_m_mtx.csv',layer_2_top_m_mtx,delimiter=',')

layer_2_top_m_mtx = np.loadtxt('layer_2_top_m_mtx.csv', delimiter=',')
...
welspd = {0:None,1:None,2:well,3:well,4:well}    # CZ241001 flopy does not like 0 anymore.
...
xs, ys = rasterio.transform.xy(base_map.transform, rows.flatten(), cols.flatten()) 
xcoords = np.array(xs)
ycoords = np.array(ys)
