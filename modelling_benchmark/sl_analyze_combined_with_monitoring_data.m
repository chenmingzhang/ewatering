...
for nt=2:6:length(nod)-1
    ...
    a.plot1=plot(time_measured_data_day(1:1000:end),...
            Tbl.sa2_groundwater_table_rise_mm(1:1000:end)*c.mPmm,'b.','displayname','Mea. at SA2' );hold on

    %a.plot1=plot(time_nod_day(1:nt),watertable_diff_sa2_m(1:nt),...
    %         'r-','linewidth',a.lw);   %the original way to getting the water head.

    a.plot1=plot(time_nod_day(1:nt) , (arrayfun(@(y) y.terms{p_idx}(point1.idx_nod-1),nod(1:nt)) - nod(1).terms{p_idx}(point1.idx_nod-1)) / c.rhow_pure_water / c.g ,'r-','linewidth',a.lw ,'displayname','simulation' );
    a.plot1=plot(time_nod_day(1:nt) , (arrayfun(@(y) y.terms{p_idx}(point1.idx_nod  ),nod(1:nt)) - nod(1).terms{p_idx}(point1.idx_nod  )) / c.rhow_pure_water / c.g ,'g-','linewidth',a.lw ,'displayname','simulation' );
    a.plot1=plot(time_nod_day(1:nt) , (arrayfun(@(y) y.terms{p_idx}(point1.idx_nod+1),nod(1:nt)) - nod(1).terms{p_idx}(point1.idx_nod+1)) / c.rhow_pure_water / c.g ,'b-','linewidth',a.lw ,'displayname','simulation' );
    a.plot1=plot(time_nod_day(1:nt) , (arrayfun(@(y) y.terms{p_idx}(point1.idx_nod+2),nod(1:nt)) - nod(1).terms{p_idx}(point1.idx_nod+2)) / c.rhow_pure_water / c.g ,'c-','linewidth',a.lw ,'displayname','simulation' );hold off

    ylabel({'Groundwater';'Level at SA2 (m)'},'FontSize',a.fs);
    legend('Location','northeast','fontsize',a.fs_legend);    
    ylim([-0.3 3]);
    xlim([0 140]);
    set(gca,'XTick',[])
    ...
    a.plot11=plot(time_measured_data_day(1:1000:end),...
            Tbl.sa1_groundwater_table_rise_mm(1:1000:end)*c.mPmm,'b.');hold on
    a.plot11=plot(time_measured_data_day(1:1000:end),...
            Tbl.sa4_groundwater_table_rise_mm(1:1000:end)*c.mPmm,'g.');        

    %a.plot11=plot(time_nod_day(1:nt),watertable_diff_sa1_m(1:nt),...
    %         'r-','linewidth',a.lw);
   

    a.plot11=plot(time_nod_day(1:nt) , (arrayfun(@(y) y.terms{p_idx}(point3.idx_nod-1),nod(1:nt)) - nod(1).terms{p_idx}(point3.idx_nod-1))  /c.rhow_pure_water/c.g ,'r-','linewidth',a.lw  );
    a.plot11=plot(time_nod_day(1:nt) , (arrayfun(@(y) y.terms{p_idx}(point3.idx_nod),nod(1:nt  )) - nod(1).terms{p_idx}(point3.idx_nod  ))  /c.rhow_pure_water/c.g ,'g-','linewidth',a.lw  );
    a.plot11=plot(time_nod_day(1:nt) , (arrayfun(@(y) y.terms{p_idx}(point3.idx_nod+1),nod(1:nt)) - nod(1).terms{p_idx}(point3.idx_nod+1))  /c.rhow_pure_water/c.g ,'b-','linewidth',a.lw  );
    a.plot11=plot(time_nod_day(1:nt) , (arrayfun(@(y) y.terms{p_idx}(point3.idx_nod+2),nod(1:nt)) - nod(1).terms{p_idx}(point3.idx_nod+2))  /c.rhow_pure_water/c.g ,'c-','linewidth',a.lw  );hold off

     txt=sprintf('Result at day %.2f , k_silt = %.2e m2, k_sand = %.2e m2\n init P in silt layer = %.2e Pa'...
    ...
    xlim([0 140]);
    set(gca,'XTick',[])
    legend('Location','southeast','fontsize',a.fs_legend);
    ...
    a.plot2=plot(time_measured_data_day(1:1000:end),...
        (Tbl.p3_cs451(1:1000:end)-0.2213),'b.');hold on   

    %a.plot2 = plot(time_nod_day(1:nt) , (arrayfun(@(y) y.terms{p_idx}(point_pond.idx_nod-1),nod(1:nt)) - nod(1).terms{p_idx}(point_pond.idx_nod-1))  /c.rhow_pure_water/c.g ,'r-','linewidth',a.lw  );
    %a.plot2 = plot(time_nod_day(1:nt) , (arrayfun(@(y) y.terms{p_idx}(point_pond.idx_nod),nod(1:nt  )) - nod(1).terms{p_idx}(point_pond.idx_nod  ))  /c.rhow_pure_water/c.g ,'g-','linewidth',a.lw  );
    %a.plot2 = plot(time_nod_day(1:nt) , (arrayfun(@(y) y.terms{p_idx}(point_pond.idx_nod+1),nod(1:nt)) - nod(1).terms{p_idx}(point_pond.idx_nod+1))  /c.rhow_pure_water/c.g ,'b-','linewidth',a.lw  );
    %a.plot2 = plot(time_nod_day(1:nt) , (arrayfun(@(y) y.terms{p_idx}(point_pond.idx_nod+2),nod(1:nt)) - nod(1).terms{p_idx}(point_pond.idx_nod+2))  /c.rhow_pure_water/c.g ,'c-','linewidth',a.lw  );

    pond_depth_integrated_m=  arrayfun(@(y) y.terms{s_idx}(point_pond.idx_nod),nod(1:nt  ))*dy_cell_mtx(point_pond.idx_nod)* inp.por(point_pond.idx_nod) + ...
         arrayfun(@(y) y.terms{s_idx}(point_pond.idx_nod+1),nod(1:nt))*dy_cell_mtx(point_pond.idx_nod)* inp.por(point_pond.idx_nod);
    %a.plot2 = plot(time_nod_day(1:nt) , arrayfun(@(y) y.terms{s_idx}(point_pond.idx_nod),nod(1:nt  ))*dy_cell_mtx(point_pond.idx_nod)* inp.por(point_pond.idx_nod),'g-','linewidth',a.lw  );
    %a.plot2 = plot(time_nod_day(1:nt) , arrayfun(@(y) y.terms{s_idx}(point_pond.idx_nod+1),nod(1:nt))*dy_cell_mtx(point_pond.idx_nod)* inp.por(point_pond.idx_nod),'b-','linewidth',a.lw  );
    %a.plot2 = plot(time_nod_day(1:nt) , arrayfun(@(y) y.terms{s_idx}(point_pond.idx_nod+2),nod(1:nt))*dy_cell_mtx(point_pond.idx_nod)* inp.por(point_pond.idx_nod),'c-','linewidth',a.lw  );
    a.plot2 = plot(time_nod_day(1:nt) , pond_depth_integrated_m,'g-','linewidth',a.lw  );hold off

    xlim([0 140]);
    ylim([0 0.9]);
    set(gca,'XTick',[])
    legend({'Measurement','simulation'},'Location','southeast','fontsize',a.fs_legend);
...
%     %legend('show','Location','East')
    ...
    axis([-1 time_nod_day(end) -0.005 0.038])
