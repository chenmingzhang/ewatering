...
dx_ele_mtx_3d_m                     = cat(3 , zeros(nez,ney,1) , diff(x_ele_mtx_3d_m , 1 , 3) );  % ?CZ first column is wrong
dz_ele_mtx_3d_m                     = cat(1 , zeros(1,ney,nex) , diff(z_ele_mtx_3d_m , 1 , 1) );  % ?CZ first column is wrong

area_xz_cell_mtx_m2_3d              = - dx_cell_mtx_3d .* dz_cell_mtx_3d;     %  CZ?
area_xz_ele_mtx_m2_3d               = - dx_ele_mtx_3d_m  .* dz_ele_mtx_3d_m ; %  CZ?

area_xy_ele_mtx_m2_2d(1,:)          = 0.5 * (y_nod_matrix_mtx_2d(1,1:end-1)+y_nod_matrix_mtx_2d(1,2:end) ) .* squeeze(diff(x_nod_mtx_2d(1,:) ));%formula to calculate the area of %CZ?
...
pump_inflow_ay_node_time_kgPs       = cell2mat(arrayfun(@(y) y.qin( 1 : length(source_point_index) ),bcof,'UniformOutput',false) );
evap_ay_node_time_kgPs              = cell2mat(arrayfun(@(y) y.qin(length(source_point_index) + 1:end),bcof,'UniformOutput',false) );
qpl                                 = cell2mat( arrayfun( @(y) y.qpl,bcop,'UniformOutput',false ) );
volume_cell_mtx_m3                  = dx_cell_mtx_3d .* abs(dy_cell_mtx_3d) .* abs(dz_cell_mtx_3d);
tout_diff_s                         = diff(tout_s);
tout_diff_s                         = [0 tout_diff_s];
input_diff_kg                       = ones(length(source_point_index),1) * tout_diff_s .* pump_inflow_ay_node_time_kgPs;
...
et_mmPday                           =c.m2mm * evap_ay_node_time_kgPs / c.rhow_pure_water ./ evap_point_area_xy_array / c.dayPsec;
...
%     h11                           = plot ( time_nod_day( 1:nt ), nod_to )
    ...
    legend('position',[fig_pos.left+4.4*fig_pos.length,...
                       fig_pos.bottom-4*fig_pos.height-5.8*fig_pos.height,...
    ...
    a.sub33           = subplot('position'...
                    ,[fig_pos.left+4.4*fig_pos.length,...
                       fig_pos.bottom-4*fig_pos.height-4.4*fig_pos.height,...
                       fig_pos.length,fig_pos.height]);         
    h8                              = plot(time_nod_day(1:nt),total_water_sum_kg(1:nt) / c.mlPl,'linewidth',a.lw,'DisplayName','Total water');
    xlim([0 floor(time_nod_day(end) )]);
    ylim([700 max(total_water_sum_kg)/c.mlPl]);
    set(gca,'linewidth',a.lw); 
    grid on;    ylabel({'Total water mass (ML)'},'FontSize',a.fs); 
    xlabel('Time (day)','FontSize',a.fs);  
    set(gca,'fontsize',a.fs,'fontweight','bold');        
         %% -------------  flux at the pond bottom  --------------   
   
    a.sub8             = subplot('position'...
                    ,[fig_pos.left+4.35*fig_pos.length,fig_pos.bottom-3*fig_pos.height-3.6*fig_pos.height...
                    fig_pos.length,fig_pos.height]); 
                
    vz        = cell2mat(arrayfun(@(y) y.terms{vz_idx},ele(nt),'UniformOutput',false) );
	vz_matrix = reshape(vz,[nez,ney,nex]);     
    pond_flux_macropore_kg_m2_day = pond_flux_ele_xy_kgPs(1:ney:end,:)./area_xy_ele_mtx_m2_3d(pond_boundary_ele_index(1:ney:end))/c.dayPsec;
    pond_flux_matrix_kg_m2_day = pond_flux_ele_xy_kgPs(2:ney:end,:)./area_xy_ele_mtx_m2_3d(pond_boundary_ele_index(1:ney:end))/c.dayPsec;
    
    [Ax , Line1 , Line2]                             = plotyy( x_ele_mtx_3d_m(pond_boundary_ele_index(1:ney:end)+2 ) , ...
                                                                [pond_flux_macropore_kg_m2_day(:,nt),...
                                                                 pond_flux_matrix_kg_m2_day(:,nt)],...                                                                                                  
                                                                x_ele_mtx_3d_m(pond_boundary_ele_index(1:ney:end)+2 ) ,...
                                                                [vz_matrix(pond_boundary_ele_index(1:ney:end)+2 ) * c.secPday,...
                                                                 vz_matrix(pond_boundary_ele_index(2:ney:end)+2 ) * c.secPday ] );
    set(Line1(1),'color','r','linestyle','-','linewidth',a.lw);
    set(Line1(2),'color','r','linestyle',':','linewidth',a.lw);
    set(Line2(1),'color','b','linestyle','-','linewidth',a.lw);
    set(Line2(2),'color','b','linestyle',':','linewidth',a.lw);   
    set(Ax(1),'YLim',[min(pond_flux_macropore_kg_m2_day(:)) max(pond_flux_macropore_kg_m2_day(:))],'YTick',[-200 -100 -10 0 10],'fontsize',a.fs,'linewidth',a.lw);
    set(Ax(2),'YLim',[-1 1],'YTick',[-1 0 1],'fontsize',a.fs,'linewidth',a.lw);
    
%     hold on;
%     h2                              = plot( x_ele_mtx_3d_m(pond_boundary_ele_index(1:ney:end) ) ,,'r:','linewidth',a.lw);
%     set(gca, 'YScale', 'log');
%     ylim([min(aquitard_flux_ele_xy_kgPs(:) )/c.dayPsec max(aquitard_flux_ele_xy_kgPs(:) )/c.dayPsec]);
    xlabel('Distance (m)','FontSize',a.fs);      
    ylabel({'Flux - pond(kg/m2/day)'},'FontSize',a.fs); 
    set(gca,'fontsize',a.fs,'fontweight','bold');        
    set(gca,'linewidth',a.lw); 
    grid on;
         %% -------------  flux at the aquifer  --------------   
    ...
    a.sub8   = subplot('position'...
                    ...
                    fig_pos.length,fig_pos.height]);        
    h1                              = plot(squeeze(x_ele_mtx_3d_m(1,1,:) ),squeeze(sum(aquitard_flux_ele_xy_kgPs(1:ney:end,nt),2) )./area_xy_ele_mtx_m2_2d(1,:)'/c.dayPsec,'r-','linewidth',a.lw);hold on;
    h2                              = plot(squeeze(x_ele_mtx_3d_m(1,1,:) ),squeeze(sum(aquitard_flux_ele_xy_kgPs(2:ney:end,nt),2) )./area_xy_ele_mtx_m2_2d(2,:)'/c.dayPsec,'r:','linewidth',a.lw);
%     xlim([0 max(squeeze(x_ele_mtx_3d_m(1,1,:) ) )]);
    ylim([-0.1 0.1]);
%     set(gca, 'YScale', 'log');
%     ylim([min(aquitard_flux_ele_xy_kgPs(:) )/c.dayPsec max(aquitard_flux_ele_xy_kgPs(:) )/c.dayPsec]);
    xlabel('Distance (m)','FontSize',a.fs);      
